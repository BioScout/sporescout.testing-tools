...
function Download-PortableArtifact {
  ...
  $runsResponse = Invoke-RestMethod -Uri $DownloadConfig.workflowRunsApi -Headers $headers -Method Get
...
$portableApp = if ($Dev) { $null } else { Find-PortableApp -Root $root -Manifest $manifest }
